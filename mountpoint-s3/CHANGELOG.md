...
* Add optional copy-rename emulation on general purpose buckets with `--allow-copy-rename` (requires `--allow-delete`). Renames committed files up to 5 GiB using `CopyObject` followed by `DeleteObject`. The operation is not atomic; see `doc/UNOFFICIAL.md`. Authorization failures after a partial apply disable copy-rename for the remainder of the mount.
