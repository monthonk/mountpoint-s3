...
pub struct PrefetchGetObject<Stream: ObjectPartStream, Client: ObjectClient> {
    ...
    // Expected next offset for backpressure read window.
    // The actual next offset could be larger than this value in some cases,
    // for example, when the client return a part larger than window size.
    ...
    Client: ObjectClient + Send + Sync + 'static,
{
    ...
    async fn read(
        ...
        while to_read > 0 {
            ...
            let next_read_window_offset = self.next_read_window_offset.max(current_task.next_read_window_offset());
            let remaining_window =
                next_read_window_offset.saturating_sub(self.next_sequential_read_offset + part.len() as u64);
            self.prepare_next_read_window(remaining_window as usize, next_read_window_offset);
    ...
    Client: ObjectClient + Send + Sync + 'static,
{
    ...
    fn new(
        ...
        PrefetchGetObject {
            ...
            next_read_window_offset: config.initial_read_window_size as u64,
    ...
    fn reset_prefetch_to_offset(&mut self, offset: u64) {
        ...
        self.next_read_window_offset = offset + self.config.initial_read_window_size as u64;
    ...
    async fn try_seek_forward(&mut self, offset: u64) -> Result<bool, PrefetchReadError<Client::ClientError>> {
        ...
        if self.next_read_window_offset < available_soon_offset {
            let diff = available_soon_offset - self.next_read_window_offset;
            task.increment_read_window(diff as usize);
            self.next_read_window_offset = available_soon_offset;
        }
    ...
    fn prepare_next_read_window(
        &mut self,
        remaining_window_size: usize,
        next_read_window_offset: u64,
    ) -> Option<usize> {
        let remaining = self.size.saturating_sub(next_read_window_offset);
        if next_read_window_offset < self.size && remaining_window_size < self.current_read_window_size / 2 {
