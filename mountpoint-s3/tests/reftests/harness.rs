...
use crate::common::{make_test_filesystem, DirectoryReply, ReadReply, TestS3Filesystem};
...
impl Harness {
    ...
    async fn check_local_file(&self, inode: InodeNo) {
        ...
        let mut read = Err(0);
        self.fs
            .read(inode, open.fh, 0, 4096, 0, None, ReadReply(&mut read))
            .await;
        let read = read.expect_err("read should fail");
        assert_eq!(read, libc::EBADF);
    }
}
